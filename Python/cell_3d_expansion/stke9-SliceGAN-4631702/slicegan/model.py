...
def train(pth, imtype, datatype, real_data, Disc, Gen, nc, l, nz, sf, data_name):
    """
    train the generator
    ...
    for epoch in range(num_epochs):
        ...
        for i, (datax, datay, dataz) in enumerate(zip(dataloaderx, dataloadery, dataloaderz), 1):
            ...
            print('here')
            if i % 1 == 0:
                torch.save(netG.state_dict(), pth + '_' + data_name + '_Gen.pt')
                torch.save(netD.state_dict(), pth + '_' + data_name + '_Disc.pt')
                ...
                ###save example slices
                util.test_plotter(img, 5, imtype, pth, data_name)
                # plotting graphs
                util.graph_plot([disc_real_log, disc_fake_log], ['real', 'perp'], pth, 'LossGraph', data_name)
                util.graph_plot([Wass_log], ['Wass Distance'], pth, 'WassGraph', data_name)
                util.graph_plot([gp_log], ['Gradient Penalty'], pth, 'GpGraph', data_name)
