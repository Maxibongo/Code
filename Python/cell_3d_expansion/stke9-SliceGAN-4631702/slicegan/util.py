...
def get_random_data_paths(all_data_path, isotropic = True):
    data_paths = []
    data_name = ''

    if isotropic:
        img = np.random.choice(os.listdir(all_data_path))
        data_paths.append(str(all_data_path)+'/'+img)
    else:
        first_img = str(np.random.choice(os.listdir(all_data_path)))
        data_paths.append(str(all_data_path)+'/'+first_img)
        for i in range(2):
            img = str(np.random.choice(os.listdir(all_data_path)))
            while img[0:6] != first_img[0:6]:
                img = str(np.random.choice(os.listdir(all_data_path)))
            data_paths.append(str(all_data_path)+'/'+img)
    ...
    return data_paths, data_name[0:len(data_name)-1]
...
def test_plotter(img,slcs,imtype,pth, data_name):
    """
    ...
    plt.savefig(pth + '_' + data_name + '_slices.png')
    plt.close()

def graph_plot(data,labels,pth,name, data_name):
    """
    ...
    plt.legend()
    plt.savefig(pth + '_' + data_name + '_' + name)
    plt.close()


def get_random_generator_name(pth):
    generator_names = [file for file in os.listdir(pth) if file.endswith('Gen.pt')]
    choice = np.random.choice(generator_names)
    return choice[0:len(choice)-7]


def test_img(pth, imtype, netG, data_name, nz = 64, lf = 4):
    """
    ...
    :return:
    """
    netG.load_state_dict(torch.load(pth + '/' + data_name + '_Gen.pt'))
    netG.eval()
    ...
    tif = np.int_(gb)
    tifffile.imwrite(pth + '/' + data_name + '.tif', tif)
